...
gdf_haz['starttime'] = pd.to_datetime(df_haz_raw['starttime'], utc=True) #format starttime
...
df_countries = df_countries[["iso3", "name", "geometry"]].rename(columns={"geometry": "country_geometry", "name": "Country"})
...
#%%
df_em_dat_raw["Start Date"] = pd.to_datetime(df_em_dat_raw[['Start Year', 'Start Month', 'Start Day']].rename(columns = {'Start Year':'year', 'Start Month': 'month', 'Start Day': 'day'}), utc=True)
df_em_dat_raw["End Date"] = pd.to_datetime(df_em_dat_raw[['End Year', 'End Month', 'End Day']].rename(columns = {'End Year':'year', 'End Month': 'month', 'End Day': 'day'}), utc=True)
...
df_em_dat_raw =   df_em_dat_raw[[
    ...
    "Latitude", 
    "Longitude"
]].rename(columns={"Start Date": "starttime", 
    "End Date":"endtime"})
...
#%% filter on location
gdf_impact = gpd.GeoDataFrame(df_impact, geometry = df_impact["best_geometry"], crs = "EPSG:4326")
...
df_haz_impact = gdf_haz.merge(right = gdf_impact, how = "cross", suffixes = ["_hazard", "_impact"])


#%%df_haz_impact
spatial_filter = gpd.GeoSeries(df_haz_impact["geometry_hazard"]).intersects(gpd.GeoSeries(df_haz_impact["geometry_impact"]))
df_haz_impact = df_haz_impact[spatial_filter]

#%%
temporal_filter = df_haz_impact.apply(lambda x: has_overlap_in_time(x["starttime_hazard"], x["endtime_hazard"], x["starttime_impact"] , x["endtime_impact"]), axis = 1)
df_haz_impact = df_haz_impact[temporal_filter]


#%% Split into a hazard and an impact gdf for plotting
gdf_haz2 = gpd.GeoDataFrame(df_haz_impact, geometry = df_haz_impact["geometry_hazard"], crs = "EPSG:4326")
gdf_imp2 = gpd.GeoDataFrame(df_haz_impact, geometry = df_haz_impact["geometry_impact"], crs = "EPSG:4326")
...
# %%
fields_rasterized = features.rasterize(shapes = [flood_geom], out_shape=ds_exp.shape[1:3], transform=ds_exp.rio.transform())
