...
for min_overlap_thres in min_overlap_thress:
    for max_time_lag in max_time_lags:
        ...
        for unique_event in unique_events_sorted:
            ...
            influencing_events = get_influencing_events(
                unique_event, event_pairs, df_emdat
            )
            ...
            while loop_condition:
                ...
                for event_new in influencing_events:
                    influencing_events_temp = (
                        influencing_events_temp
                        + get_influencing_events(event_new, event_pairs, df_emdat)
                    )
        ...
        df_chain = pd.DataFrame.from_dict(
            dict_all_events, orient="index", columns=["Events"]
        )
        ...
        for ix, row in df_chain.iterrows():
            ...
            for event in events:
                hazards = hazards + list(
                    df_emdat.loc[event, ["Hazard1", "Hazard2", "Hazard3"]]
                    .dropna()
                    .values
                )
