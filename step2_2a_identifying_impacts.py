...
df_events = pd.read_csv(
    "data/df_events_" + str(min_overlap_thres) + "_" + str(max_time_lag) + ".csv",
    sep=";",
    index_col=0,
)
...
# %% Create empty dataframe for single-hazard and hazard-pair impacts
cols = [
    "Dis No 1",
    "Dis No 2",
    "Hazard 1",
...
for ix, row in df_events.loc[
    df_events["No hazards"] <= 2
].iterrows():  # Extract those events that report 1 or 2 hazards (this solves the duplicate issue)
    dis_no1 = row["Records"][0]
